...
public final class QuickLZ15Optimized {
      ...
      if (level == 1) {
        ...
        if (cache == 0 && hash_counter[hash] != 0 && (src - o > MINOFFSET || (src == o + 1 && lits >= 3 && src > 3 && source[src] == source[src - 3] && source[src] == source[src - 2] && source[src] == source[src - 1] && source[src] == source[src + 1] && source[src] == source[src + 2]))) {
          ...
          if (source[o + 3] != source[src + 3]) {
            int f = 3 - 2 | (hash << 4);
            dest[dst + 0] = (byte) (f >>> 0 * 8);
            dest[dst + 1] = (byte) (f >>> 1 * 8);
            src += 3;
            dst += 2;
          } else {
            int old_src = src;
...
//            fast_write(dest, dst, ((matchlen - 3) << 2) | (offset << 6) | 2, 2);
            ...
            fast_write3(dest, dst, ((matchlen - 2) << 2) | (offset << 7) | 3);
            dst += 3;
          } else {
            fast_write4(dest, dst, ((matchlen - 3) << 7) | (offset << 15) | 3);
            dst += 4;
          }
        } else {
          dest[dst] = source[src];
...
//        fast_write(dest, cword_ptr, (long)((cword_val >>> 1) | 0x80000000L), 4);
  ...
  static int fast_read3(byte[] a, int i) {
    return (a[i] & 0xff) | ((a[i+1] & 0xff) << 8) | ((a[i+2] & 0xff) << 16);
  }
  ...
  static void fast_write1(byte[] a, int i, long value) {
    a[i] = (byte) value;
  }

  static void fast_write2(byte[] a, int i, long value) {
    a[i] = (byte) value;
    a[i+1] = (byte) (value >>> 8);
  }

  static void fast_write3(byte[] a, int i, long value) {
    a[i] = (byte) value;
    a[i+1] = (byte) (value >>> 8);
    a[i+2] = (byte) (value >>> 16);
  }

  static void fast_write4(byte[] a, int i, long value) {
    a[i] = (byte) value;
    a[i+1] = (byte) (value >>> 8);
    a[i+2] = (byte) (value >>> 16);
    a[i+3] = (byte) (value >>> 24);
